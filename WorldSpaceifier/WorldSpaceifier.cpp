...
//=============================================================================
// $File: //depot/3darg/Tools/NormalMapper/WorldSpaceifier.cpp $ $Revision: #3 $ $Author: gosselin $
//=============================================================================
...
//////////////////////////////////////////////////////////////////////////
static void
ReadNormalMap(char* name, int* width, int* height, int* numComponents, float** normals)
{
    // Check parameters
    if ((name == NULL) || (width == NULL) || (height == NULL) || (numComponents == NULL) ||
        (normals == NULL)) {
...
//////////////////////////////////////////////////////////////////////////
static void
ProcessArgs(int argc, char** argv, char** lowName, char** tsName, char** outName, double* bumpScale)
{
    // A common usage line so everything is consistent.
    char* flags = NULL;
    static char* usage = "Usage: WorldSpaceifier [-BeEgGhHimMpPqQt] model normal/bumpmapName "
                         "[heightscale] outputname\n";
    ...
    if (argc == 6) {
        ...
        flags = argv[1];
        if (!gToWorldSpace) {
            NmPrint("ERROR: -t (convert to tangent space) option doesn't make sense with a height "
                    "map!\n");
            exit(-1);
        }
    }
...
main(int argc, char** argv)
{
    ...
    int numComponents = 3;
    if (gBumpMap) {
        GetBumpMapFromHeightMap(tsName, &gWidth, &gHeight, &tsNormals, bumpScale);
    } else {
                    ...
                    for (int x = (minX - pad); x <= (maxX + pad); x++) {
                        ...
                        for (int t = 0; t < 9; t++) {
                            ts[t] = (tangentSpace[l].m[0][t] * b1) +
                                    (tangentSpace[l].m[1][t] * b2) + (tangentSpace[l].m[2][t] * b3);
                        }
                        ...
                        Fetch(tsNormals,
                              fullWidth,
                              fullHeight,
                              (double)(x) / (double)(fullWidth),
                              (double)(y) / (double)(fullHeight),
                              sNorm);
                        if (gToWorldSpace) {
