...
//////////////////////////////////////////////////////////////////////////
static inline bool
IntersectionIsBetter(int rule,
                     NmRawPointD* norm,
                     double nNorm[3],
                     NmRawPointD* nIntersect,
                     double lNorm[3],
                     NmRawPointD* lIntersect)
{
#ifdef _DEBUG
    ...
    if ((norm == NULL) || (nNorm == NULL) || (nIntersect == NULL) || (lNorm == NULL) ||
        (lIntersect == NULL)) {
    ...
    if (VEC_DotProduct(nNorm, norm->v) > gMaxAngle) {
        ...
        switch (rule) {
            ...
            case NORM_RULE_FRONT_CLOSEST:
                ...
                if ((nIntersect->x >= -gEpsilon) && (nIntersect->x < gDistance) &&
            ...
            case NORM_RULE_BEST_CLOSEST:
                ...
                if ((fabs(nIntersect->x) < gDistance) &&
                    ...
                    return true;
                           ...
                           (VEC_DotProduct(nNorm, norm->v) > VEC_DotProduct(lNorm, norm->v))) {
                    return true;
                }
                return false;
            ...
            case NORM_RULE_FRONT_BEST_CLOSEST:
                ...
                if ((nIntersect->x >= -gEpsilon) && (nIntersect->x < gDistance) &&
                    ((nIntersect->x < lIntersect->x) || first)) {
                    return true;
                } else if ((nIntersect->x < gDistance) && (nIntersect->x == lIntersect->x) &&
                           (VEC_DotProduct(nNorm, norm->v) > VEC_DotProduct(lNorm, norm->v))) {
                    return true;
                }
                return false;
...
{
#ifdef _DEBUG
    if ((orig == NULL) || (dir == NULL) || (v1 == NULL) || (v2 == NULL) || (v3 == NULL) ||
...
///////////////////////////////////////////////////////////////////////////////
static inline bool
RayIntersectsBox(NmRawPointD* position, NmRawPointD* direction, AtiOctBoundingBox* box)
{
#ifdef _DEBUG
...
TriInBox(int32 itemIndex, void* itemList, AtiOctBoundingBox* bbox)
{
    ...
    return AtiTriBoxMoller(bbox->center,
                           bbox->half,
                           tri[itemIndex].vert[0].v,
                           tri[itemIndex].vert[1].v,
                           tri[itemIndex].vert[2].v);
}
...
//////////////////////////////////////////////////////////////////////////////
static bool
GetRays(int numDivisions, int* numRays, NmRawPointD** rays, double** rayWeight)
{
    // Check parameters
...
FromToRotation(double mtx[16], double from[3], double to[3])
{
    ...
    if (f > 1.0f - EPSILON) /* "from" and "to"-vector almost parallel */
    {
        ...
        for (i = 0; i < 3; i++) {
            for (j = 0; j < 3; j++) {
                mtx[i + j * 4] = -c1 * u[i] * u[j] - c2 * v[i] * v[j] + c3 * v[i] * u[j];
            }
            mtx[i + i * 4] += 1.0f;
        }
...
//////////////////////////////////////////////////////////////////////////
static void
GetBumpMapFromHeightMap(char* bumpName,
                        int* bumpWidth,
                        int* bumpHeight,
                        float** bumpMap,
                        float scale)
...
//////////////////////////////////////////////////////////////////////////
static void
ReadModel(char* name, char* type, int* numTris, NmRawTriangle** tris, double bbox[6], bool checkTex)
{
    // Check arguments
    ...
    for (int i = 0; i < (*numTris); i++) {
        for (int j = 0; j < 3; j++) {
            ...
            if (checkTex) {
                if (((*tris)[i].texCoord[j].u < 0.0) || ((*tris)[i].texCoord[j].u > 1.0)) {
                    if (fabs((*tris)[i].texCoord[j].u) > EPSILON) {
                        NmPrint("ERROR: Texture coordinates must lie in the 0.0 - 1.0 range for "
                                "the %s res model (u: %f)!\n",
                                type,
                                (*tris)[i].texCoord[j].u);
                        exit(-1);
                ...
                if (((*tris)[i].texCoord[j].v < 0.0) || ((*tris)[i].texCoord[j].v > 1.0)) {
...
//////////////////////////////////////////////////////////////////////////
static void
GenerateTangents(NmTangentMatrix** tangentSpace, int numTris, NmRawTriangle* tris)
{
    // Check arguments
...
//////////////////////////////////////////////////////////////////////////
static inline void
GetPerturbedNormal(NmRawTriangle* tri,
                   double b0,
                   double b1,
                   double b2,
                   float* bumpMap,
                   ...
                   double norm[3])
{
    ...
    double u = (tri->texCoord[0].u * b0) + (tri->texCoord[1].u * b1) + (tri->texCoord[2].u * b2);
    double v = (tri->texCoord[0].v * b0) + (tri->texCoord[1].v * b1) + (tri->texCoord[2].v * b2);
...
{
#ifdef _DEBUG
    if ((newPos == NULL) || (newNorm == NULL) || (tri == NULL) || (octree == NULL) ||
        (rays == NULL) || (rayWeights == NULL) || (bentNormal == NULL) || (occlusion == NULL)) {
    ...
    for (int r = 0; r < numRays; r++) {
        ...
        while ((numCells > 0) && !intersect) {
            ...
            else { // Non-leaf, add the children to the list if their bounding
...
//////////////////////////////////////////////////////////////////////////
static inline bool
FindBestIntersection(NmRawPointD& pos,
                     NmRawPointD& norm,
                     AtiOctree* octree,
        ...
        if (leaf) {
            ...
            for (int t = 0; t < currCell->m_numItems; t++) {
                ...
                if (IntersectTriangle(pos.v,
                    ...
                    if (IntersectionIsBetter(
                          gNormalRules, &norm, nn, &intersect, newNorm, &lastIntersect)) {
#ifdef DEBUG_INTERSECTION
                        if (gDbgIntersection) {
                        ...
                        memcpy(&lastIntersect, &intersect, sizeof(NmRawPointD));
...
CopyEdges(float* img1, float* img2, int numComponents)
{
    ...
    for (int y = 0; y < gHeight; y++) {
        ...
        for (int x = 0; x < gWidth; x++) {
            ...
            if (((fabs(img1[idx]) < EPSILON) && (fabs(img1[idx + 1]) < EPSILON) &&
                 (fabs(img1[idx + 2]) < EPSILON)) &&
                ((fabs(img2[idx]) >= EPSILON) || (fabs(img2[idx + 1]) >= EPSILON) ||
...
NormalizeImage(float* img, int numComponents)
{
    ...
    for (int y = 0; y < gHeight; y++) {
        ...
        for (int x = 0; x < gWidth; x++) {
            // See if this pixel is empty
            int idx = y * gWidth * numComponents + x * numComponents;
            if ((fabs(img[idx]) >= EPSILON) || (fabs(img[idx + 1]) >= EPSILON) ||
        ...
        for (int y = 0; y < gHeight; y++) {
            ...
            for (int x = 0; x < gWidth; x++) {
                ...
                if ((fabs((*img)[idx]) <= EPSILON) && (fabs((*img)[idx + 1]) <= EPSILON) &&
                    ...
                    if ((fabs((*img)[nidx]) > EPSILON) || (fabs((*img)[nidx + 1]) > EPSILON) ||
...
BoxFilter(float* result, float* source, int lastWidth, int numComponents)
{
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            oldIdx =
              ((y * mult) + 1) * lastWidth * numComponents + ((x * mult) + 1) * numComponents;
            norm[0] += source[oldIdx];
            norm[1] += source[oldIdx + 1];
            norm[2] += source[oldIdx + 2];
            if (gOcclusion) {
...
#define MASK11 0x7ff
                    outImg[idx] =
                      ((((unsigned long)(img[fIdx] * MASK9) + MASK9) & MASK10) << 22) |
...
TestArgs(char* args)
{
    ...
    if ((strstr(args, "w") != NULL) || (strstr(args, "W") != NULL)) {
    ...
    if (gOcclusion) {
        if (gOutput == NORM_OUTPUT_8_8_8_TGA) {
            ...
            NmPrint("Warning: Specified output format will NOT output occlusion. Right now only "
                    "8x8x8x8 TARGA (default) and 16x16x16x16 ARG (-H) will output occlusion!\n");
        }
    }
    if (strstr(args, "B") != NULL) {
        gBoxFilter = true;
            ...
            char** outName)
{
    ...
    char* flags = NULL;
    static char* usage = "Usage: NormalMapper [-0123456789aAbBcCdDeEfFgGhHiklLmMnNoOpPqQrRtTvwXyz] "
                         "lowres highres Width Height [value] [heightmap heightscale] outputname\n";
...
main(int argc, char** argv)
{
    ...
    char* outName = NULL;
    ProcessArgs(argc, argv, &lowName, &highName, &bumpName, &bumpScale, &outName);
    ...
    if (bumpName != NULL) {
        ...
        GetBumpMapFromHeightMap(bumpName, &bumpWidth, &bumpHeight, &bumpMap, (float)bumpScale);
    }
    ...
    if (octree->FillTree(
          highNumTris, (void*)(highTris), bbox, TriInBox, gMaxInBox, NmOctreeProgress) == FALSE) {
                    ...
                    for (int x = (minX - pad); x <= (maxX + pad); x++) {
                        ...
                        if (gDbgIntersection) {
                            ...
                            NmPrint("      n1:   %8.4f  %8.4f  %8.4f\n",
                                    ...
                                    lTri->norm[0].z);
                            NmPrint(
                              "      t1: %8.4f  %8.4f\n", lTri->texCoord[0].u, lTri->texCoord[0].v);
                            ...
                            NmPrint("      n2:   %8.4f  %8.4f  %8.4f\n",
                                    ...
                                    lTri->norm[1].z);
                            NmPrint(
                              "      t2: %8.4f  %8.4f\n", lTri->texCoord[1].u, lTri->texCoord[1].v);
                            ...
                            NmPrint("      n3:   %8.4f  %8.4f  %8.4f\n",
                                    ...
                                    lTri->norm[2].z);
                            NmPrint(
                              "      t3: %8.4f  %8.4f\n", lTri->texCoord[2].u, lTri->texCoord[2].v);
                        ...
                        for (int s = 0; s < gNumSamples; s++) {
                            ...
                            if ((b1 > 1.0) || (b1 < 0.0) || (b2 > 1.0) || (b2 < 0.0) ||
                                (b3 > 1.0) || (b3 < 0.0)) {
#ifdef DEBUG_INTERSECTION
                                if (gDbgIntersection) {
