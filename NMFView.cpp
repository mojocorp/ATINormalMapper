...
//=============================================================================
// $File: //depot/3darg/Tools/NormalMapper/NMFView.cpp $ $Revision: #14 $ $Author: rdhiraj $
//=============================================================================
...
LoadObjectFile(char* objectFile)
{
    ...
    if (!NmCreateVertexBuffers(gNumTris, gTriangles, &gNumVerts, &gVertices, &gIndices)) {
        printf("ERROR: Unable to read file: %s\n", objectFile);
        gNumTris = 0;
        delete[] gTriangles;
        gTriangles = NULL;
...
MouseMove(int x, int y)
{
    ...
    if (gRightDown) {
        ...
        GLfloat m[16];
        matrixInvert(gViewMat, m);
        GLfloat v[3] = { 0.0f, 0.0f, ((x - gRightX) * 0.1f + (y - gRightY) * 0.1f) };
        GLfloat o[3];
        vecMat3x3Mult(v, m, o);
        glPushMatrix();
        glLoadIdentity();
...
main(int argc, char** argv)
{
    ...
    printf(" a              - Cycle showing normal map, diffuse bump, or vertex lit, or occlusion "
           "term (if present)\n");
