...
NmWriteTriangles(FILE* fp, int numTris, NmRawTriangle* tris)
{
    ...
    header.size = sizeof(NmHeader) + sizeof(int) + (sizeof(NmRawTriangle) * numTris);
...
NmReadTriangleChunk(FILE* fp, int* numTris, NmRawTriangle** tris)
{
    ...
    if (fread((*tris), sizeof(NmRawTriangle), (*numTris), fp) != (unsigned)(*numTris)) {
...
#define DcVec2MidPoint(c, a, b)                                                                    \
    ((r)[0] = (b)[0] + ((a)[0] - (b)[0]) * 0.5f, (r)[1] = (b)[1] + ((a)[1] - (b)[1]) * 0.5f)
...
#define DcVec2NormalizeDouble(v)                                                                   \
    {                                                                                              \
        double __mag = 1.0 / DcVec2MagnitudeDouble(v);                                             \
        (v)[0] *= __mag;                                                                           \
        (v)[1] *= __mag;                                                                           \
    }
#define DcVec2Normalize(v)                                                                         \
    {                                                                                              \
        float __mag = 1.0f / (float)DcVec2Magnitude(v);                                            \
        (v)[0] *= __mag;                                                                           \
        (v)[1] *= __mag;                                                                           \
    }

#define DcVecAdd(r, a, b)                                                                          \
    ((r)[0] = (a)[0] + (b)[0], (r)[1] = (a)[1] + (b)[1], (r)[2] = (a)[2] + (b)[2])
#define DcVecAddTo(a, b) ((a)[0] += (b)[0], (a)[1] += (b)[1], (a)[2] += (b)[2])
#define DcVecAddScalar(r, a, b)                                                                    \
    ((r)[0] = (a)[0] + (b), (r)[1] = (a)[1] + (b), (r)[2] = (a)[2] + (b))
#define DcVecAddScalarTo(a, b) ((a)[0] += (b), (a)[1] += (b), (a)[2] += (b))
#define DcVecCopy(a, b) ((b)[0] = (a)[0], (b)[1] += (a)[1], (b)[2] += (a)[2])
#define DcVecCross(r, a, b)                                                                        \
    ((r)[0] = (a)[1] * (b)[2] - (a)[2] * (b)[1],                                                   \
     (r)[1] = (a)[2] * (b)[0] - (a)[0] * (b)[2],                                                   \
     (r)[2] = (a)[0] * (b)[1] - (a)[1] * (b)[0])
#define DcVecDivide(r, a, b)                                                                       \
    ((r)[0] = (a)[0] / (b)[0], (r)[1] = (a)[1] / (b)[1], (r)[2] = (a)[2] / (b)[2])
...
#define DcVecMidPoint(c, a, b)                                                                     \
    ((r)[0] = (b)[0] + ((a)[0] - (b)[0]) * 0.5f,                                                   \
     (r)[1] = (b)[1] + ((a)[1] - (b)[1]) * 0.5f,                                                   \
     (r)[2] = (b)[2] + ((a)[2] - (b)[2]) * 0.5f)
#define DcVecMult(a, b, c)                                                                         \
...
#define DcVecNormalizeDouble(v, def)                                                               \
    {                                                                                              \
        double __mag = DcVecMagnitudeDouble(v);                                                    \
        if (__mag < EPSILON) {                                                                     \
            (v)[0] = (def)[0];                                                                     \
            (v)[1] = (def)[1];                                                                     \
            (v)[2] = (def)[2];                                                                     \
...
#define DcVecNormalize(v)                                                                          \
    {                                                                                              \
        float __mag = 1.0f / (float)DcVecMagnitude(v);                                             \
        (v)[0] *= __mag;                                                                           \
        (v)[1] *= __mag;                                                                           \
        (v)[2] *= __mag;                                                                           \
    }
#define DcVecLineLength(a, b)                                                                      \
    (sqrtf(((a)[0] - (b)[0]) * ((a)[0] - (b)[0]) + ((a)[1] - (b)[1]) * ((a)[1] - (b)[1]) +         \
           ((a)[2] - (b)[2]) * ((a)[2] - (b)[2])))
#define DcVecLerp(r, a, b, c)                                                                      \
    {                                                                                              \
        DcVecSubtract(r, b, a);                                                                    \
               ...
               double* binormal)
{
    ...
    //===================================//
    if ((v1 != NULL) && (v2 != NULL) && (v3 != NULL) && (t1 != NULL) && (t2 != NULL) &&
        (t3 != NULL) && (normal != NULL) && (tangent != NULL) && (binormal != NULL)) {
        //========================================//
...
////////////////////////////////////////////////////////////////////
static void
ComputeTangentVectorsD(NmRawTriangle* pgon,
                       int idx,
                       double tan[3],
                       double norm[3],
                       double binorm[3])
{
    ...
    if (pgon->texCoord[0].u == 0.0 && pgon->texCoord[1].u == 0.0 && pgon->texCoord[2].u == 0.0) {
        return;
    }

    DcTangentSpace(pgon->vert[0].v,
                   pgon->vert[1].v,
                   pgon->vert[2].v,
                   pgon->texCoord[0].uv,
                   pgon->texCoord[1].uv,
...
////////////////////////////////////////////////////////////////////
static void
NmCopyPointTangentD(NmRawTriangle* tri, NmRawTangentSpaceD* tan, int v, NmTangentPointD* point)
{
    point->vertex[0] = tri->vert[v].x;
    point->vertex[1] = tri->vert[v].y;
    point->vertex[2] = tri->vert[v].z;
...
////////////////////////////////////////////////////////////////////
static int
NmInsertD(NmRawTriangle* tri, int v, int* num, int* sortIndex, NmTangentPointD* point)
{
    ...
    if ((*num) > 0) {
        ...
        int compValue;
#ifdef NEW_WAY
        int pos = NmIndexBinaryTraverseD(pt, point, sortIndex, (*num), &compValue, NmCompareNoTexD);
#else
        int pos = NmIndexBinaryTraverseD(pt, point, sortIndex, (*num), &compValue, NmCompareD);
#endif
        ...
        {
            ret = (*num);
            memmove(&(sortIndex[pos + 1]), &(sortIndex[pos]), ((*num) - pos) * sizeof(int));
        ...
        } else // we are appending after this index
        {
            ...
            if (pos < ((*num) - 1)) {
                memmove(
                  &(sortIndex[pos + 2]), &(sortIndex[pos + 1]), ((*num) - pos - 1) * sizeof(int));
            }
            sortIndex[pos + 1] = (*num);
            (*num)++;
        }
    } else { // First point just add it into our list.
...
////////////////////////////////////////////////////////////////////
static int
NmInsertForVBD(NmRawTriangle* tri,
               ...
               NmTangentPointD* point)
{
    ...
    if ((*num) > 0) {
        ...
        int compValue;
        int pos =
          NmIndexBinaryTraverseD(pt, point, sortIndex, (*num), &compValue, NmCompareWithTexD);
                      ...
                      NmIndex** indices)
{
    ...
    for (i = 0; i < numTris; i++) {
        for (int j = 0; j < 3; j++) {
#ifdef NEW_WAY
            index[i].idx[j] = NmInsertForVBD(&tris[i], &tan[i], j, &count, sortIndex, point);
#else
            index[i].idx[j] = NmInsertD(&tris[i], j, &count, sortIndex, point);
#endif
        }
