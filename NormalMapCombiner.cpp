...
//=============================================================================
// $File: //depot/3darg/Tools/NormalMapper/NormalMapCombiner.cpp $ $Revision: #2 $ $Author: rdhiraj
// $
//=============================================================================
            ...
            double* bumpScale)
{
    ...
    char* flags = NULL;
    static char* usage = "Usage: NormalMapCombiner [-BeEgGhHimMqQ] normalmap1 bump/normalMap2 "
...
main(int argc, char** argv)
{
    ...
    int height = 0;
    if (gBumpMap) {
        GetBumpMapFromHeightMap(nm2Name, &width, &height, &nm2Normals, bumpScale);
    } else {
        int nc = 3;
        ReadNormalMap(nm2Name, &width, &height, &nc, &nm2Normals);
        if (nc != numComponents) {
            NmPrint(
              "Normal maps must have the same number of components (%d %d)!\n", numComponents, nc);
            exit(-1);
        }
    }
