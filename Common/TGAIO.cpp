...
////////////////////////////////////////////////////////////////////
bool
TGAWriteImage(FILE* fp, int width, int height, int bitDepth, uint8* image, bool aSwapRedBlue)
{
    int i;
    int bytesRead;
    ...
    if ((bitDepth != 24) && (bitDepth != 32)) {
        fprintf(
          stderr, "ERROR! Can only write 24 and 32 bit images! This image is %d bits.\n", bitDepth);
        return false;
    }
    ...
    for (i = 0; i < (width * height); i++) {
        if (aSwapRedBlue == false) { // Note: TGA is swapped by default
            ptr2[0] = ptr1[2];
            ptr2[1] = ptr1[1];
            ptr2[2] = ptr1[0];
        } else {
    ...
    for (i = 0; i < width * height * bitDepth / 8; i += 8192000) {
        ...
        if ((bytesRead = fwrite(&texels[i], sizeof(uint8), j, fp)) != j) {
            fprintf(stderr,
                    "ERROR! Texels not written successfully! Wrote %d instead of  %d\n",
                    bytesRead,
                    j);
            return false;
        }
